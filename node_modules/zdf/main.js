...
module.exports = {
  getSources: function(url, handle, handleInfo) {
    request(url, function(context) {
      var jsb = getJSB(context);
      jsb.apiToken = jsb.apiToken ? jsb.apiToken : getAPIToken(context);
      var configuration = {};
      request("www.zdf.de" + jsb.config, function(result) {
        ...
        request(jsb.content.replace("https://", ""), function(result) {
          ...
          var info = {
            ...
            teasertext: videoInfo.teasertext,
            preview: videoInfo.teaserImageRef.layouts.original
          }
          if (handleInfo != undefined) handleInfo(info);
          var url = "api.zdf.de/tmd/2/ngplayer_2_3/vod/ptmd/mediathek/" + assetId;
  ...
  downloadStream: function(url, output, handle, streamInfo) {
    if (output == undefined) {
...
function getAPIToken(context) {
  var search = context.match(/apiToken:(\ )?'([^']*)'/);
  return search[2];
}
